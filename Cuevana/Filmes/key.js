...
// ============= MARCADORES =============
let markers = JSON.parse(localStorage.getItem('customMarkers')) || ["Filmes"];
...
function saveMarkers() {
    const inputs = document.querySelectorAll('#markers-list input');
    markers = Array.from(inputs).map(input => input.value.trim()).filter(v => v);
    localStorage.setItem('customMarkers', JSON.stringify(markers));
}

// Abrir modal
document.getElementById('openMarkers')?.addEventListener('click', () => {
    document.getElementById('markers-modal').style.display = 'flex';
    renderMarkers();
});

// Fechar modal
document.getElementById('closeMarkersModal')?.addEventListener('click', () => {
    document.getElementById('markers-modal').style.display = 'none';
});
...
document.getElementById('markers-list')?.addEventListener('click', (e) => {
...
document.getElementById('markers-list')?.addEventListener('input', (e) => {
...
document.getElementById('markers-modal')?.addEventListener('click', (e) => {
    if (e.target.id === 'markers-modal') {
        document.getElementById('markers-modal').style.display = 'none';
    }
});